...
function initAnimation(obj) {
    ...
    var PATTERNS__MODE = "PATTERNS__MODE",
    ...
    var myLoadList = obj.patterns,
        myLoadListWithPath = obj.patterns.map(function (el) {
        return obj.path2patterns + el;
    }),
    ...
    function onAllLoaded(Pattern_array) {
        ...
        typeWriter(init_text, 0, function () {
            butt__styleit.disabled = false;
        });
    }
    ...
    butt__styleit.onclick = function () {
        //clicked "Stile it!" button
    ...
    butt__refont.onclick = function () {
        ...
        var currentFont = getStylePropertyValue(lineArr[0], "font-family");
        var currentN = fontsArr.indexOf(currentFont);
        currentFontIndex = currentN + 1 < fontsArr.length ? currentN + 1 : 0;

        lineArr.forEach(function (el) {
            el.style.fontFamily = fontsArr[currentFontIndex];
        });
        setTimeout(function () {
            redrawOutput(inputText.value);
        }, 100);
...
function fit_in(target, object_txt) {
    //where (Snap.select("#...")), what
